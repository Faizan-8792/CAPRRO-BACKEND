  ...
  document.querySelectorAll('.task-delay-reason').forEach((sel) => {
    sel.addEventListener('change', async () => {
      ...
      try {
        ...
        // 🔥 CRITICAL FIX: Refresh both the task board AND compliance assistant
        await refreshTaskBoard();
        await loadAdminComplianceAssistant();
      } catch (error) {
        console.error('Failed to update delay reason:', error);
        alert('Failed to update delay reason: ' + (error.message || 'Unknown error'));
      }
    });
  });
}
